...
1. 何时需要手动分配栈空间
2. assume 可以不需要吗？
3. 如何在汇编中 debug 到某个具体的某个地址，不然一直 -p 手都按麻了
4. 如果代码用到了 stack，debug 的时候具体怎么情形会到值 debug 结果和执行执行 结果不一致，不一致的时候如何找 bug 呢
5. 汇编子程序如何带参数
6. 子程序在修改寄存器的值何时应该保存，什么时候不应该保存
7. 如何做多文件汇编？
